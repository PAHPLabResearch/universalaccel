...
if (length(runs)) {
  ...
  preview <- function(pattern) {
    ...
    readr::read_csv(f[1], n_max = 5, show_col_types = FALSE,
...
knitr::kable(preview_n(1) |> dplyr::select(1:9))
```
...
knitr::kable(preview_n(2) |> dplyr::select(1:9))
```
...
knitr::kable(preview_n(3) |> dplyr::select(1:9))
```
...
knitr::kable(preview_n(4) |> dplyr::select(1:9))
```
