...
**universalaccel (UA)** is an R pipeline for computing harmonized accelerometer summary metrics across multiple devices (Tested on: ActiGraph, Axivity, GENEActiv devices proprietary outputs, but there is an option of processing generic outputs from other devices (eg. triaxial data in csv, rds formats, etc). The goal is to make raw accelerometer data comparable by producing standardized, epoch-level outputs with consistent naming, time handling, and reproducible settings.

### UA tested files and order of operations:

- ActiGraph (GT3X), .gt3x raw data is passed through `?agcounts::GGIR` first for autocalibration and metrics are computed.
- Axivity (AX3), .resampled.CSV is autocalibrated in OmGui if .cwa files are loaded, agcounts::agcalibrate is used to autocalibrate.
- GeneActiv, .bin raw data is passed through `?GENEAread::recalibrate()` first and metrics are computed.
- generic, the workflow accepts files from other devices in formats (csv, csv.gz, rds, rda/RData, xlsx/xls, parquet, feather) provided that they have four columns indicating (time, and triaxial values (X, Y, Z) that are regularly sampled). For autocalibration, generic data uses the agcalibrate() function of the agcounts.  
...
- Raw device files (ActiGraph `.gt3x`, Axivity `resampled`.csv` and `.cwa` , GENEActiv `.bin`, and generic `.csv` formats)

### Key expectations
- A consistent alignment of metric at the selected epoch length (`epochs`; commonly 5 seconds or 60 seconds).
- Correct sample rate and dynamic range for raw devices as configured (or when required by some summary metrics).
- Consistent timezone handling (typically this is handled by device software but it is configurable here [example argument: tz = "UTC"]).
- Optional autocalibration for generic devices (generic_autocalibrate = "True")
...
accel_summaries(
  device        = "actigraph", #define device (axivity, geneactiv or generic) 
  data_folder   = actigraph_path, # data folder for batch processing
  output_folder = output_dir, # output folder
  epochs        = 60, # can optionally compute at many epochs c( 1, 5, 60)
  sample_rate   = 100,# adjust as needed (Note that for COUNTS >=30 is required)
  dynamic_range = c(-8, 8), # adjust as needed
  metrics       = c("MIMS","ENMO","MAD","COUNTS","AI","ROCAM"),
  apply_nonwear = TRUE
  
# other arguments including (generic_autocalibrate = "True/False",
# generic_verbose= TRUE/FALSE, and tz = "timezone") can be added. 
)
...
### Output5-6 — MX metrics 

**Purpose:** The threshold intensity above which the most active X minutes were accumulated (daily and weekly).

### Output7 — Run report (log)
