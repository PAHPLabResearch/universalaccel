...
# Run Part 2 analysis (writes a folder named "<pre_base>_UA_outputs" under out_root)
analysis <- ua_analyze_precomputed(
  in_path     = pre_csv,
  out_dir     = out_root,   # parent folder (function creates <pre_base>_UA_outputs)
  location    = "ndw",
  make_weekly = TRUE,
  overwrite   = TRUE
)
...
preview <- function(pattern) { 
  ...
  if (length(f)) readr::read_csv(f[1], n_max = 5) }
